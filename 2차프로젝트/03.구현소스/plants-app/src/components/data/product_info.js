export const productInfo = {
  main: {
    tit1: "WHAT ARE HERBAL TINCTURES?",
    desc: "Herbal tinctures are highly bioavailable concentrated essence of medicinal herbs.",
    tit2: "",
    images: [
      {
        isrc: "/gifs/Organic.gif",
        itxt: "100% Organic",
      },
      {
        isrc: "/gifs/NonGMO.gif",
        itxt: "Non GMO",
      },
      {
        isrc: "/gifs/Nosugar.gif",
        itxt: "No added sugar",
      },
      {
        isrc: "/gifs/Nosyntheticflavours.gif",
        itxt: "No synthetic flavour",
      },
      {
        isrc: "/gifs/Noartificialcolours.gif",
        itxt: "No artificial colours",
      },
      {
        isrc: "/gifs/Vegan.gif",
        itxt: "Vegan",
      },
    ],
  },
  about: {
    tit1: "",
    desc: "Our Standards",
    tit2: "",
    images: [
      {
        isrc: "/gifs/Organic.gif",
        itxt: "100% Organic",
      },
      {
        isrc: "/gifs/NonGMO.gif",
        itxt: "Non GMO",
      },
      {
        isrc: "/gifs/Nosugar.gif",
        itxt: "No added sugar",
      },
      {
        isrc: "/gifs/Nosyntheticflavours.gif",
        itxt: "No synthetic flavour",
      },
      {
        isrc: "/gifs/Noartificialcolours.gif",
        itxt: "No artificial colours",
      },
      {
        isrc: "/gifs/Vegan.gif",
        itxt: "Vegan",
      },
    ],
  },
  shop: {
    tit1: "WHAT ARE HERBAL TINCTURES?",
    desc: "Herbal tinctures are highly bioavailable concentrated essence of medicinal herbs.",
    tit2: "",
    images: [
      {
        isrc: "/gifs/Organic.gif",
        itxt: "100% Organic",
      },
      {
        isrc: "/gifs/NonGMO.gif",
        itxt: "Non GMO",
      },
      {
        isrc: "/gifs/Nosugar.gif",
        itxt: "No added sugar",
      },
      {
        isrc: "/gifs/Nosyntheticflavours.gif",
        itxt: "No synthetic flavour",
      },
      {
        isrc: "/gifs/Noartificialcolours.gif",
        itxt: "No artificial colours",
      },
      {
        isrc: "/gifs/Vegan.gif",
        itxt: "Vegan",
      },
    ],
  },
  calm: {
    tit1: "KEY BENEFITS",
    desc: "Calming nervines to help you find a sense of peace on stressful days",
    tit2: "WHEN TO USE",
    images: [
      {
        isrc: "/gifs/Bag.gif",
        itxt: "Keep it in your bag for on-the-go support",
      },
      {
        isrc: "/gifs/Meditativeday.gif",
        itxt: "For a gentle start to your day",
      },
      {
        isrc: "/gifs/Travel.gif",
        itxt: "Your travel-friendly anxiety support",
      },
      {
        isrc: "/gifs/Moon.gif",
        itxt: "Take to wind down after a stressful day",
      },
      {
        isrc: "/images/Meditation.png",
        itxt: "A ritual before your meditation practice",
      },
    ],
  },
  shroomBoost: {
    tit1: "KEY BENEFITS",
    desc: "Cognitive support, energy, mood, anti‑inflammatory*",
    tit2: "MAKE IT A RITUAL",
    images: [
      {
        isrc: "/gifs/Bag.gif",
        itxt: "Keep it in your bag for on-the-go support",
      },
      {
        isrc: "/gifs/Meditativeday.gif",
        itxt: "Use when you need the energy without the crash",
      },
      {
        isrc: "/gifs/coffee.gif",
        itxt: "Add it to your tea/coffee for an adaptogenic boost",
      },
      {
        isrc: "/gifs/Brain.gif",
        itxt: "A daily ritual for long term cognitive benefits",
      },
    ],
  },
  sleep: {
    tit1: "KEY BENEFITS",
    desc: "Lets racing minds catch a break, setting the stage for sweet dreams*",
    tit2: "MAKE IT A RITUAL",
    images: [
      {
        isrc: "/gifs/Bed.gif",
        itxt: "Keep it by your bedside for a nightly ritual",
      },
      {
        isrc: "/gifs/Anxiety.png",
        itxt: "Soothe your central nervous system",
      },
      {
        isrc: "/gifs/Moon.gif",
        itxt: "Take to wind down after a stressful day",
      },
      {
        isrc: "/gifs/Travel.gif",
        itxt: "Your travel-friendly jetlag support",
      },
    ],
  },
  digest: {
    tit1: "KEY BENEFITS",
    desc: "Stimulates digestion, eases bloating, soothes the gut*",
    tit2: "WHEN TO USE",
    images: [
      {
        isrc: "/gifs/Candle.gif",
        itxt: "Your anti flatulence support on dinner dates ;)",
      },
      {
        isrc: "/gifs/AdventurousEater.gif",
        itxt: "A faithful ally for adventurous eaters",
      },
      {
        isrc: "/gifs/Travel.gif",
        itxt: "A digestive saviour on exotic holidays",
      },
      {
        isrc: "/gifs/Bag.gif",
        itxt: "Your on-the-go digestive support",
      },
    ],
  },
  theRescueBundle: {
    tit1: "KEY BENEFITS",
    desc: "Embrace restful sleep, soothe anxiety, support your gut's wellbeing, & fuel your energy naturally*",
    tit2: "",
    images: [
      {
        isrc: "/gifs/Organic.gif",
        itxt: "100% Organic",
      },
      {
        isrc: "/gifs/NonGMO.gif",
        itxt: "Non GMO",
      },
      {
        isrc: "/gifs/Nosugar.gif",
        itxt: "No added sugar",
      },
      {
        isrc: "/gifs/Nosyntheticflavours.gif",
        itxt: "No synthetic flavour",
      },
      {
        isrc: "/gifs/Noartificialcolours.gif",
        itxt: "No artificial colours",
      },
      {
        isrc: "/gifs/Vegan.gif",
        itxt: "Vegan",
      },
    ],
  },
  sleepRescueBundle: {
    tit1: "KEY BENEFITS",
    desc: "Keep your nerves steady during the day, let them ease into relaxation at night.",
    tit2: "",
    images: [
      {
        isrc: "/gifs/Organic.gif",
        itxt: "100% Organic",
      },
      {
        isrc: "/gifs/NonGMO.gif",
        itxt: "Non GMO",
      },
      {
        isrc: "/gifs/Nosugar.gif",
        itxt: "No added sugar",
      },
      {
        isrc: "/gifs/Nosyntheticflavours.gif",
        itxt: "No synthetic flavour",
      },
      {
        isrc: "/gifs/Noartificialcolours.gif",
        itxt: "No artificial colours",
      },
      {
        isrc: "/gifs/Vegan.gif",
        itxt: "Vegan",
      },
    ],
  },
  gutRescueBundle: {
    tit1: "KEY BENEFITS",
    desc: "Soothe your mind. Soothe your gut. Protect that mind‑gut connection.",
    tit2: "",
    images: [
      {
        isrc: "/gifs/Organic.gif",
        itxt: "100% Organic",
      },
      {
        isrc: "/gifs/NonGMO.gif",
        itxt: "Non GMO",
      },
      {
        isrc: "/gifs/Nosugar.gif",
        itxt: "No added sugar",
      },
      {
        isrc: "/gifs/Nosyntheticflavours.gif",
        itxt: "No synthetic flavour",
      },
      {
        isrc: "/gifs/Noartificialcolours.gif",
        itxt: "No artificial colours",
      },
      {
        isrc: "/gifs/Vegan.gif",
        itxt: "Vegan",
      },
    ],
  },
  anxietyRescueBundle: {
    tit1: "KEY BENEFITS",
    desc: "Nervous system support for calmer days, clearer minds & balanced energy",
    tit2: "",
    images: [
      {
        isrc: "/gifs/Organic.gif",
        itxt: "100% Organic",
      },
      {
        isrc: "/gifs/NonGMO.gif",
        itxt: "Non GMO",
      },
      {
        isrc: "/gifs/Nosugar.gif",
        itxt: "No added sugar",
      },
      {
        isrc: "/gifs/Nosyntheticflavours.gif",
        itxt: "No synthetic flavour",
      },
      {
        isrc: "/gifs/Noartificialcolours.gif",
        itxt: "No artificial colours",
      },
      {
        isrc: "/gifs/Vegan.gif",
        itxt: "Vegan",
      },
    ],
  },
};
